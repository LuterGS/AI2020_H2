...
def load_dict(f_name):
    ...
    with open(os.path.join(f_name),'r',encoding='utf8') as f:
        dictionary = []
...
def convert_sent2dict(sent, max_length):  # baseline load_data 함수에서의 sentence 변수가 첫번째 인자로 그대로 들어가면 됨
    ...
    return dict_onehot[:max_length]


def load_data(f_name):
    
    file = open(os.path.join(f_name), 'r', encoding='utf8')
    
    dict_counts = []
    
    for line in tqdm(file.readlines()):
        try:
            _, sentence, tags = line.strip().split('\t')
        except:
            _, sentence = line.strip().split('\t')

        dict_count = convert_sent2dict(sentence, 120)
        dict_counts.append(dict_count)

    print(len(dict_counts))
    print(dict_counts[:10])

    return np.array(dict_counts, dtype='float32')
    
    
loc_dict = load_dict("LOC.txt")
org_dict = load_dict("ORG.txt")
per_dict = load_dict("PER.txt")

onehots = load_data("ner_train.txt")


import pickle

with open('train_dict_ngrams.pickle', 'wb') as handle:
    pickle.dump(onehots, handle, protocol=pickle.HIGHEST_PROTOCOL)
